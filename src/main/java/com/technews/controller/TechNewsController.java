...
public class TechNewsController {
    ...
    public String login(@ModelAttribute User user, Model model, HttpServletRequest request) throws Exception {
        ...
        //populate sessionUser variable by identifying email of user
        ...
        //set loggedIn variable to true and return the dashboard.html
        sessionUser.setLoggedIn(true);
        request.getSession().setAttribute("SESSION_USER", sessionUser);

        return "redirect:/dashboard";
    }
//    route allows users to create new profile
    @PostMapping("/users")
...
//     allow users to add a post from their dashboard
    public String addPostDashboardPage(@ModelAttribute Post post, Model model, HttpServletRequest request) {
        ...
        //if session not valid redirect user to login
        if (request.getSession(false) == null) {
            return "redirect:/login";
        } else {
            //if session is valid and post has been submitted set userId for that post to be id of the sessionUser
